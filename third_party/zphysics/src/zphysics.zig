...
pub const GlobalState = struct {
    ...
    mem_mutex: std.atomic.Mutex = .unlocked,
...
//
//--------------------------------------------------------------------------------------------------
fn lockMemAllocator() void {
    while (!state.?.mem_mutex.tryLock()) {
        std.atomic.spinLoopHint();
    }
}

fn unlockMemAllocator() void {
    state.?.mem_mutex.unlock();
}

fn zphysicsAlloc(size: usize) callconv(.c) ?*anyopaque {
    lockMemAllocator();
    defer unlockMemAllocator();
