...
const Files = [_][]const u8{ "AtkinsonHyperlegible-Regular.ttf", "AtkinsonHyperlegible-OFL.txt" };

const Options = struct {
    source: []const u8 = "third_party/fonts",
    target: []const u8 = "third_party/fonts",
    replace: bool = false,
};
...
fn printUsage() void {
    std.debug.print(
        \\usage: zig run .agents/skills/vendor-fonts/scripts/vendor_fonts.zig -- [options]
        \\
        \\Options:
        \\  --source <path>   Directory containing Atkinson Hyperlegible files
        \\  --target <path>   Destination directory (default: third_party/fonts)
...
fn run(allocator: std.mem.Allocator, io: std.Io, options: Options) !void {
    ...
    const files = try readFiles(allocator, io, cwd, options.source);
    ...
    for (files) |file| {
        const path = try std.fs.path.join(allocator, &.{ options.target, file.path });
    ...
    std.debug.print("vendored fonts from {s} into {s}\n", .{ options.source, options.target });
}

fn readFiles(allocator: std.mem.Allocator, io: std.Io, cwd: std.Io.Dir, root: []const u8) ![]FileBlob {
    const files = try allocator.alloc(FileBlob, Files.len);
    for (Files, 0..) |name, i| {
...
fn writeProvenance(allocator: std.mem.Allocator, io: std.Io, cwd: std.Io.Dir, target: []const u8, source: []const u8) !void {
    ...
    const text = try std.fmt.allocPrint(allocator,
        ...
        \\
        \\friendly-engine vendors this font for editor and runtime UI text.
        \\
        \\To refresh this tree, use the `vendor-fonts` contributor skill in
        \\`.agents/skills/vendor-fonts`.
        \\
    , .{source});
